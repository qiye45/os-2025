...
func TestIsValidPlayer(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if result != tt.expected {
			t.Errorf("IsValidPlayer(%c) = %v, expected %v", tt.playerID, result, tt.expected)
		}
	}
}
...
func TestFindPlayer(t *testing.T) {
	...
	if pos.Row != -1 || pos.Col != -1 {
		t.Errorf("FindPlayer('2') = (%d, %d), expected (-1, -1)", pos.Row, pos.Col)
	}
}
...
func TestIsConnected(t *testing.T) {
	...
	if !IsConnected(connected) {
		t.Error("IsConnected() = false, expected true for connected maze")
	}
	...
	if IsConnected(disconnected) {
		t.Error("IsConnected() = true, expected false for disconnected maze")
	}
}
...
func TestLoadAndSaveMap(t *testing.T) {
	...
	// 测试加载地图
	lab := &Labyrinth{}
	if !LoadMap(lab, testFile) {
		t.Error("LoadMap() failed")
		return
	}
	...
	if !SaveMap(lab, saveFile) {
		t.Error("SaveMap() failed")
		return
	}
...
func TestMovePlayer(t *testing.T) {
	...
	// 测试向右移动
	if !MovePlayer(lab, '0', "right") {
		t.Error("MovePlayer(right) failed")
	}

	// 验证玩家位置
	pos := FindPlayer(lab, '0')
	if pos.Row != 0 || pos.Col != 2 {
